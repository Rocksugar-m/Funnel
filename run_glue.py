...
def main():
    ...
    parser.add_argument("--is_sparse", default=False, action="store_true",
                        help="using sparse model to evaluating")
    ...
    if args.is_sparse :
        assert not training_args.do_train, "args.is_sparse requires args.do_train=False"
        assert training_args.do_eval, "args.is_sparse requires args.do_eval=True"
    ...
    if data_args.pad_to_max_length:
        data_collator = default_data_collator # DataCollatorWithPadding(tokenizer, padding='max_length', max_length=256)# 解决MRPC最长序列无法padding到512的问题
    ...
    if training_args.do_eval:
        ...
        if args.eval_checkpoint is not None:
            ...
            trainer.model_wrapped.load_state_dict(state_dict, False)
