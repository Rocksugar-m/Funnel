...
class BertSelfAttention(nn.Module):
    ...
    def forward(
        ...
        for i in range(1, len(thresholds_q)):
            quantized_query_layer = torch.where((query_layer >= thresholds_q[i-1]) & (query_layer < thresholds_q[i]), values[i], quantized_query_layer)
        quantized_query_layer = torch.where(query_layer >= thresholds_q[13], values[14], quantized_query_layer)
        
        quantized_key_layer = torch.where(key_layer < thresholds_k[0], values[0], key_layer)
        for i in range(1, len(thresholds_k)):
